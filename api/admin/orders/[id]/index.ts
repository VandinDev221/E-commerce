...
/**
 * Rota explícita para GET /api/admin/orders/:id (dados do pedido para etiqueta).
 * Evita 404 quando o catch-all não repassa o path corretamente na Vercel.
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  const id = (req as any).query?.id;
  if (!id || typeof id !== 'string') {
    res.status(400).json({ error: 'ID do pedido ausente' });
    return;
  }
  const normalized = `/api/admin/orders/${id}`;
