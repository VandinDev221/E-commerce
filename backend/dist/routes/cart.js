...
function getCartIdentifier(req) {
    ...
    const sessionId = req.headers['x-cart-session'] || req.cookies?.cartSession;
    if (sessionId)
        return { sessionId };
    return {};
}
...
router.get('/', optionalAuth, async (req, res, next) => {
    try {
        const id = getCartIdentifier(req);
        if (!id.userId && !id.sessionId)
            return res.json({ items: [], subtotal: 0, isUser: false });
...
router.post('/add', optionalAuth, async (req, res, next) => {
    try {
        const id = getCartIdentifier(req);
        if (!id.userId && !id.sessionId)
            throw new AppError('Envie header x-cart-session ou faça login', 400);
...
router.patch('/:itemId', optionalAuth, async (req, res, next) => {
    try {
        const id = getCartIdentifier(req);
        if (!id.userId && !id.sessionId)
            throw new AppError('Carrinho não identificado', 400);
...
router.post('/sync', authMiddleware, async (req, res, next) => {
    try {
        ...
        if (!req.user || !sessionId)
            return res.json(await loadCart(req));
